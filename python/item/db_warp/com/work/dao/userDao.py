from com.work.util import DBUtil
from com.work.model import model
from com.work.model import Page
...
def insert(user):
    ...
    try:
        cursor = conn.cursor()
        sql = "insert into user (name, province, city, address, zip, data) value (%s, %s, %s, %s, %s, %s)"
...
def update(user):
    ...
    try:
        cursor = conn.cursor()
        sql = "update user set name=%s, province=%s, city=%s, address=%s, zip=%s, data=%s where id=%s"
        cursor.execute(sql, user)
        conn.commit()
        return cursor.rowcount
    finally:
