window.onload = function () {
    init(function (obj) {
        ...
        tr.insertCell(-1).innerHTML = json.stuid;
        tr.insertCell(-1).innerHTML = json.name;
        tr.insertCell(-1).innerHTML = json.sex;
        tr.insertCell(-1).innerHTML = json.age;
        console.log(json);
    });
}
...
function init(dle) {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            dle(xhr.responseText); //响应的是json格式文本:'{"stuid": 1,"name": "张三","sex": "男","age": 18}'
            // console.log(jsonText, typeof jsonText);
            //将json格式的文本解析成js对象
            //(1)早期eval() -- 要加上“（）”
            // var obj1 = eval('(' + jsonText + ')');
            ...
            //(2)全局对象JSON.parse()
            // return dle = JSON.parse(jsonText);
            // console.log(obj2, obj2.stuid, obj2.name);
